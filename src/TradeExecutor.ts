...
import { withRetry, edgexCircuitBreaker, paradexCircuitBreaker } from './utils/retry';

export class TradeExecutor extends EventEmitter {
  private edgexProcess: ChildProcess | null = null;
  private paradexProcess: ChildProcess | null = null;

  private edgexReady: boolean = false;
  private paradexReady: boolean = false;
   ...
   * 启动 EdgeX 交易服务
   */
  private async startEdgeXService(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.edgexProcess.stdout?.on('data', (data: Buffer) => {
      ...
      this.edgexProcess.stderr?.on('data', (data: Buffer) => {
   ...
   * 启动 Paradex 交易服务
   */
  private async startParadexService(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.paradexProcess.stdout?.on('data', (data: Buffer) => {
        const lines = data.toString().split('\n').filter(line => line.trim());
      ...
      this.paradexProcess.stderr?.on('data', (data: Buffer) => {
        const log = data.toString();
        if (log.includes('ERROR') || log.includes('✅') || log.includes('🚀')) {
   ...
   * 处理 EdgeX 消息
   */
  private handleEdgeXMessage(message: any): void {
   ...
   * 处理 Paradex 消息
   */
  private handleParadexMessage(message: any): void {
    if (message.type === 'command_result') {
      const callback = this.commandCallbacks.get(message.data.id);
      if (callback) {
        this.commandCallbacks.delete(message.data.id);
   ...
   * 发送命令到 EdgeX
   */
  async sendEdgeXCommand(action: string, params: any): Promise<any> {
    return new Promise((resolve, reject) => {
   ...
   * 发送命令到 Paradex
   */
  async sendParadexCommand(action: string, params: any): Promise<any> {
    return new Promise((resolve, reject) => {
   ...
   * 获取 EdgeX 当前价格
   */
  async getEdgeXPrice(): Promise<any> {
    try {
   ...
   * 获取 Paradex 当前价格
   */
  async getParadexPrice(): Promise<any> {
    try {
   ...
   * EdgeX 下单（带智能重试和熔断保护）
   */
  async placeEdgeXOrder(side: OrderSide, amount: number): Promise<TradeResult> {
    try {
   ...
   */
  async placeParadexOrder(side: OrderSide, amount: number): Promise<TradeResult> {
    try {
      ...
      // 处理成交数据
      const fillData: ParadexFillData = {
        id: result.fill.fillId,
        order_id: result.id,
