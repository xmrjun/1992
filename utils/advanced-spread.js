 ...
 */
export function calculateFillPrice(side, size, orderbook) {
    // 参数验证
    if (size <= 0) {
    ...
    if (remainingSize > 1e-8) { // 允许微小的浮点误差
        console.warn(`[calculateFillPrice] 深度不足: 需要 ${size}, 只能成交 ${filledSize} (${side})`);
        return NaN;
    }
    // 返回加权平均价
 ...
 */
export function computeExecutableSpreads(size, exchangeABook, exchangeBBook) {
    // 计算四个方向的加权成交价
 ...
 */
export function ema(prev, current, alpha = 0.35) {
    // 参数验证
    if (alpha < 0 || alpha > 1) {
 ...
 * }
 */
export function shouldOpen(spreads, emaState, params, context) {
    // 默认参数
    const { minNetOpen = 25, hysteresis = 7, maxVenueSpread = 15, maxAsymmetry = 20, minUnwindLimit = -15 } = params;
    const { lastTradeTime = 0, cooldownPeriod = 10000 } = context;
    // 1. 冷静期检查
 ...
 * 用于日志输出和调试
 */
export function formatSpreads(spreads) {
    return [
 ...
 * 创建默认EMA状态
 */
export function createDefaultEMAState() {
    return {
        openA: null,
        openB: null
    };
 ...
 * 返回true表示格式有效
 */
export function validateOrderBook(book) {
    ...
    if (!Array.isArray(firstBid) || firstBid.length < 2)
        return false;
    if (!Array.isArray(firstAsk) || firstAsk.length < 2)
        return false;
    // 检查价格和数量是否为数字
    if (typeof firstBid[0] !== 'number' || typeof firstBid[1] !== 'number')
        return false;
    if (typeof firstAsk[0] !== 'number' || typeof firstAsk[1] !== 'number')
        return false;
    return true;
}
// ==================== 导出所有功能 ====================
